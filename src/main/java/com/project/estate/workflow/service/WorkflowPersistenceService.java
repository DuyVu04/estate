...
/**
 * Service Component encapsulating database persistence operations for Workflow Instances and Audit
 * Histories.
 */
@Slf4j
@Service
...
public class WorkflowPersistenceService {
  ...
  public WorkflowInstance saveOrUpdateInstance(WorkflowContext context) {
    ...
    WorkflowInstance instance =
        findInstance(context.getWorkflowName(), context.getTargetId())
            .orElseGet(
                () ->
  ...
  public void saveHistory(
      WorkflowContext context, WorkflowHistoryStatus status, String errorMessage) {
    if (context.getTargetId() == null || context.getTargetId().isBlank()) {
      return;
    }
    ...
    WorkflowHistory history =
        WorkflowHistory.builder()
            ...
            .previousStatus(
                context.getPreviousStatus() != null ? context.getPreviousStatus().name() : "NONE")
