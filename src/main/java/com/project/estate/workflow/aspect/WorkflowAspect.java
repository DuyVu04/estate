...
/**
 * Clean Aspect intercepting methods marked with @WorkflowEngine. Delegates execution lifecycle
 * directly to WorkflowStrategy.
 */
@Slf4j
@Aspect
@Component
...
public class WorkflowAspect {
  ...
  @AfterReturning(
      value = "@annotation(workflowEngine)",
      returning = "result",
      argNames = "joinPoint,result,workflowEngine")
  public void afterReturnWorkflow(
      JoinPoint joinPoint, Object result, WorkflowEngine workflowEngine) {
    var action = workflowEngine.action();
    var targetIdSpel = workflowEngine.targetIdSpel();
  ...
  @AfterThrowing(
      value = "@annotation(workflowEngine)",
      throwing = "ex",
      argNames = "joinPoint,ex,workflowEngine")
