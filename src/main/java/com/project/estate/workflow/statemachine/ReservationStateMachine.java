...
public class ReservationStateMachine {
     ...
     */
    public static Optional<TransitionRule> findRule(ReservationStatus currentStatus, ReservationAction action) {
        return TRANSITION_RULES.stream()
                .filter(rule -> Objects.equals(rule.getCurrentStatus(), currentStatus) && rule.getAction() == action)
                .findFirst();
    }

    /**
     * Validates transition and returns the next status.
     * Throws AppException if transition is invalid or actor is unauthorized.
     */
    public static ReservationStatus getNextState(ReservationStatus currentStatus, ReservationAction action, ReservationActor actor) {
        TransitionRule rule = findRule(currentStatus, action)
     ...
     */
    public static boolean isValidTransition(ReservationStatus currentStatus, ReservationAction action, ReservationActor actor) {
