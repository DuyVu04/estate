...
public class EmailConsumer {
  ...
  public void receive(
    ...
    log.info("Receive email={}, retryCount={}", message.email(), message.retryCount());
    try {
      ...
      try {
        ...
        if (message.retryCount() >= 3) {
          ...
          log.error(
              "Publish to DLQ: email={}, retryCount={}", message.email(), message.retryCount());

        } else {

          rabbitTemplate.convertAndSend(retryExchange, retryRoutingKey, message.nextRetry());
        }
  ...
  @RabbitListener(queues = "${rabbitmq.queue.email}")
  public void receiveDepositPaid(
      DepositPaidMessage message, Channel channel, @Header(AmqpHeaders.DELIVERY_TAG) long tag) {
    ...
    try {
      ...
      try {
        ...
        if (message.retryCount() >= 3) {
          ...
          rabbitTemplate.convertAndSend(deadExchange, deadRoutingKey, message);
          log.error(
              "Publish to DLQ: email={}, retryCount={}", message.userEmail(), message.retryCount());
