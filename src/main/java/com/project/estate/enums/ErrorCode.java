...
public enum ErrorCode {
  ...
  INVALID_VERIFICATION_TOKEN(
      1106, "Verification token is invalid or expired", HttpStatus.BAD_REQUEST),
  ...
  ROLE_NOT_FOUND(1112, "Role not found", HttpStatus.NOT_FOUND),

  PROPERTY_NOT_FOUND(2100, "Property not found", HttpStatus.NOT_FOUND),

  UNAUTHENTICATED(1200, "Unauthenticated", HttpStatus.UNAUTHORIZED),

  ACCESS_DENIED(1201, "Access denied", HttpStatus.FORBIDDEN),

  RESOURCE_NOT_FOUND(1300, "Resource not found", HttpStatus.NOT_FOUND),

  TOO_MANY_REQUESTS(
      1301, "Too many login attempts. Please try again later.", HttpStatus.TOO_MANY_REQUESTS),
  PROPERTY_NOT_AVAILABLE(1302, "Property not available", HttpStatus.NOT_FOUND),

  PROPERTY_ALREADY_RESERVED(1303, "Property is already reserved", HttpStatus.NOT_FOUND),

  INVALID_STATE_TRANSITION(1400, "Invalid workflow state transition", HttpStatus.BAD_REQUEST),

  UNAUTHORIZED_WORKFLOW_ACTOR(
      1401, "Actor is not authorized for this workflow transition", HttpStatus.FORBIDDEN),

  PAYMENT_NOT_FOUND(1500, "Payment not found", HttpStatus.NOT_FOUND),

  PAYMENT_ALREADY_PROCESSED(1501, "Payment has already been processed", HttpStatus.BAD_REQUEST),
