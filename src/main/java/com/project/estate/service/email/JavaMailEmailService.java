...
/**
 * Primary low-level SMTP MailService implementation using Spring JavaMailSender. Fully implements
...
public class JavaMailEmailService implements MailService {
  ...
  public void sendWithAttachments(
    ...
    try {
      ...
      log.error(
          "[JAVA_MAIL] Failed to send email with attachments to {}: {}", to, e.getMessage(), e);
      throw new AppException(ErrorCode.EMAIL_SENDING_FAILED);
    }
  }

  private void setRecipientsAndSender(MimeMessageHelper helper, String to, String subject)
      throws Exception {
