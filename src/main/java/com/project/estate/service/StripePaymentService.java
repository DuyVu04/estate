 ...
 *
 * Idempotency is guaranteed at 3 levels:
 * 1. Stripe SDK: IdempotencyKey sent with every create() call prevents duplicate sessions.
 * 2. DB: Unique constraint on payments.idempotency_key prevents duplicate payment records.
 * 3. Webhook: Checks existing Payment status before processing, returns early if already SUCCESS.
 */
@Slf4j
@Service
...
public class StripePaymentService {
     ...
     * The reservation's depositAmount is used as the payment amount.
     *
     * Idempotency: Uses "STRIPE-RES-{reservationId}" as both the DB idempotencyKey
     * and the Stripe API IdempotencyKey, ensuring that retrying this call
     * for the same reservation returns the same Stripe Session.
     */
    @Transactional
    public PaymentResponse createCheckoutSession(InitiatePaymentRequest request) {
        ...
        if (existingPayment.isPresent()) {
            log.info("[STRIPE] Returning existing checkout session for idempotencyKey={}", idempotencyKey);
            PaymentResponse response = paymentMapper.toResponse(existingPayment.get());
            response.setCheckoutUrl(existingPayment.get().getTransactionRef() != null
        ...
        BigDecimal amount = reservation.getDepositAmount() != null
        ...
        try {
                    ...
                    .addLineItem(SessionCreateParams.LineItem.builder()
                            ...
                            .setPriceData(SessionCreateParams.LineItem.PriceData.builder()
                                    ...
                                    .setProductData(SessionCreateParams.LineItem.PriceData.ProductData.builder()
                                            ...
                                            .setDescription("Tiền cọc giữ chỗ bất động sản - Mã: " + reservation.getId())
                                            .build())
                                    .build())
                            .build())
                    .build();
            ...
            log.info("[STRIPE] Checkout session created: sessionId={}, reservationId={}, amount={}",
                    session.getId(), reservation.getId(), amount);
     ...
     * Handles "checkout.session.completed" to transition reservation to DEPOSIT_PAID.
     *
     * Idempotency is guaranteed by checking the Payment status before processing.
     * If payment is already SUCCESS, returns immediately without re-processing.
     */
    public void processWebhook(String payload, String sigHeader) {
        ...
        log.info("[STRIPE_WEBHOOK] Processing checkout.session.completed: sessionId={}, reservationId={}",
                session.getId(), reservationId);

        // 4. Idempotency Check: Find payment by Stripe Session ID (transactionRef)
        String idempotencyKey = session.getMetadata() != null
        ...
        if (existingPayment.isPresent() && existingPayment.get().getStatus() == PaymentStatus.SUCCESS) {
            log.warn("[STRIPE_WEBHOOK] Duplicate event detected for idempotencyKey={}. Skipping.", idempotencyKey);
            return;
        }
        ...
        log.info("[STRIPE_WEBHOOK] Successfully processed. Reservation {} -> DEPOSIT_PAID", reservationId);
    }

    /**
     * Updates Payment record status to SUCCESS. Runs in its own transaction
     * so it commits BEFORE payDeposit() is called.
     */
    @Transactional
    public void updatePaymentToSuccess(String idempotencyKey, String sessionId, String reservationId) {
        ...
        if (paymentOpt.isPresent()) {
            ...
            payment = Payment.builder()
                    .reservation(reservation)
                    .amount(reservation.getDepositAmount() != null
                            ? reservation.getDepositAmount()
    ...
    /**
     * Retrieves the checkout URL for an existing Stripe session.
     * Used when returning an idempotent response for a duplicate initiate request.
     */
    private String retrieveSessionUrl(String sessionId) {
        try {
            ...
            log.warn("[STRIPE] Could not retrieve session URL for sessionId={}: {}", sessionId, e.getMessage());
            return null;
        }
    }
     ...
     *
     * Use case: If webhook is delayed or fails, Frontend proactively confirms payment
     * by providing the Stripe sessionId. Backend calls Stripe API to verify payment status.
     *
     * Idempotency: If webhook already processed this payment (status == SUCCESS),
     * this method returns the existing PaymentResponse without re-processing.
     *
    ...
    public PaymentResponse confirmPayment(String sessionId) {
        ...
        if (reservationId == null) {
            log.error("[STRIPE_CONFIRM] No reservationId found in session metadata for sessionId={}", sessionId);
        ...
        var existingPayment = paymentRepository.findByIdempotencyKey(idempotencyKey);
        if (existingPayment.isPresent() && existingPayment.get().getStatus() == PaymentStatus.SUCCESS) {
            log.info("[STRIPE_CONFIRM] Payment already confirmed (likely by webhook). Returning existing response.");
            return paymentMapper.toResponse(existingPayment.get());
        }

        // 4. Verify Stripe says payment is actually paid
        if (!"paid".equals(session.getPaymentStatus())) {
            log.warn("[STRIPE_CONFIRM] Session {} payment_status is '{}', not 'paid'. Returning PENDING.",
        ...
        log.info("[STRIPE_CONFIRM] Payment verified as 'paid'. Processing confirmation for reservationId={}", reservationId);
        ...
        log.info("[STRIPE_CONFIRM] Successfully confirmed. Reservation {} -> DEPOSIT_PAID", reservationId);
