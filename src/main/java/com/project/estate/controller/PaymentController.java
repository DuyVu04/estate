...
public class PaymentController {
  ...
  /**
   * Creates a Stripe Checkout Session for the given reservation. Returns a checkoutUrl that the
   * Frontend redirects the customer to.
   */
  ...
  /**
   * Receives Stripe Webhook events. Stripe sends the raw JSON payload with a Stripe-Signature
   * header for verification. This endpoint must accept raw String body (not parsed JSON) for
  ...
  /**
   * Fallback Confirm Endpoint — Called by Frontend after Stripe redirects to success page. Accepts
   * session_id or sessionId via Query Parameter (?session_id=... or ?sessionId=...).
   */
