...
class ReservationServiceWorkflowTest {
  ...
  void setUp() {
    ...
    CompleteReservationWorkflowStrategy completeStrategy =
    ...
    List<WorkflowStrategy> strategies =
        Arrays.asList(createStrategy, cancelStrategy, completeStrategy, expireStrategy);
  ...
  void reserveProperty_TriggersWorkflowEngine() {
    ...
    when(reservationRepository.existsByPropertyIdAndStatus("prop-01", ReservationStatus.ACTIVE))
        .thenReturn(false);
